---
title: "Welcome to some of my data visualization projects"
format:
  html:
    embed-resources: true
engine: knitr
---

### Individual Emergency Shelter Beds Per Homeless Individual


::: columns 

::: column width="45%" 

When looking at emergency shelter capacity relative to the homeless population, clear regional disparities emerge. States shown in red face shortages, with fewer than one shelter bed per homeless individual, while blue states indicate relative surplus capacity. This visualization highlights structural differences in how states address homelessness and emergency housing needs. 

::: 

::: column width="55%"

```{r}
#| echo: false
#| message: false
#| warning: false

packages <- c("tidyverse", "ggplot2", "plotly", "usmap")
invisible(lapply(packages, library, character.only = TRUE))
rm(packages)

HomeState <- read.csv("Data&Code/HomPopState.csv")

HomeStateUse <- HomeState |>
  mutate(total = rowSums(across(4:11), na.rm = TRUE),
         BedPer = total / HPop,
         region = tolower(State))

states_map <- map_data("state") |>
  left_join(HomeStateUse, by = "region")

ggplotly(
  ggplot(states_map, aes(
    long, lat, group = group,
    fill = BedPer,
    text = paste0(
      "State: ", tools::toTitleCase(region),
      "<br>Emergency Shelter Beds per Homeless Individual: ",
      round(BedPer, 2)
    )
  )) +
    geom_polygon(color = "black", linewidth = 0.3) +
    coord_fixed(1.3) +
    scale_fill_gradient2(
      low = "#570C18",
      high = "#000087",
      midpoint = 1,
      labels = scales::number_format(accuracy = 0.01),
      name = "Beds per Individual"
    ) +
    labs(
      title = "Emergency Shelter Beds per Homeless Individual (2024)",
      subtitle = "Red = shortage (<1), Blue = surplus (>1)"
    ) +
    theme_void(),
  tooltip = "text"
)
```

::: 

:::